...
export class JefeEditarUsuarioComponent implements OnInit {
  public identity: any={};
  ...
  eliminarUsuario() {
    ...
    .then( data => {
      this.router.navigate(['/jefe/usuarios']);
  ...
  editUser(){
      ...
      this.userService.actualizar_usuario(this.empleados)
        .then(response => {
          this.reasignarJefesOperarios(this.empleados.id, "Usuario actualizado correctamente");
          this.router.navigate(['/jefe/usuarios']);
